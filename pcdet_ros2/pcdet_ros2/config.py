...
def merge_new_config(config, new_config, root_path):
    if '_BASE_CONFIG_' in new_config:
        with open(root_path + "/" + new_config['_BASE_CONFIG_'], 'r') as f:
            try:
    ...
    for key, val in new_config.items():
        ...
        if key not in config:
            config[key] = EasyDict()
        merge_new_config(config[key], val, root_path)

    return config


def cfg_from_yaml_file(cfg_file, config, root_path):
    with open(cfg_file, 'r') as f:
        ...
        merge_new_config(config=config, new_config=new_config, root_path=root_path)
